<!-- =========================================================================================
  File Name: ReceptionistAddNew.vue
  Description: Receptionist module add new
  ----------------------------------------------------------------------------------------
...
<template>
 <div class="vx-col md:w-1/1 w-full mb-base">
    <form  @submit.prevent="editModal ? updateReceptionist() : createReceptionist()">
      <div class="vx-row">
          ...
          <div class="vx-row">
            <div class="vx-col md:w-1/2 w-full">
              ...
              <select id="nurses" name="user_id" class="custom-select w-full custom-select-lg" v-model="form.user_id">
                  <option value="" disabled selected>Receptionist</option>
        ...
        <div class="vx-col w-full mt-4"  style="text-align:center;">
            <vs-button v-show="!editMode" ref="loadableButton" id="button-with-loading" class="vs-con-loading__container" @click="createReceptionist();" type="relief">Create</vs-button>
            <vs-button v-show="editMode"  ref="loadableButton" id="button-with-loadings" class="vs-con-loading__container" @click="updateReceptionist();" type="relief">Update</vs-button>
        </div>
    </div>
    </form>
 </div>
...
export default {
  ...
  methods: {
    ...
    showCreateSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Receptionist has Created',
        text: 'Receptionist has created successfully!'
      })
    },
    showUpdatedSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Receptionist has Updated',
        text: 'Receptionist has Updated successfully!',
        globalPosition: 'top right',
      })
    },
    createReceptionist() {
      this.$store.dispatch('addReceptionist', Object.assign({}, this.form))
      this.clearFields()
      this.showCreateSuccess()
    },
    updateReceptionist(){
         this.$store.dispatch('updateReceptionist', Object.assign({}, this.form))
