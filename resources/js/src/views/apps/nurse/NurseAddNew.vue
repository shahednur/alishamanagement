<!-- =========================================================================================
  File Name: NurseAddNew.vue
  Description: Nurse module add new
  ----------------------------------------------------------------------------------------
...
<template>
 <div class="vx-col md:w-1/1 w-full mb-base">
            <form  @submit.prevent="editModal ? updateNurse() : createNurse()">
              <div class="vx-row">
                  ...
                  <div class="vx-row">
                    <div class="vx-col md:w-1/2 w-full">
                      ...
                      <select id="nurses" name="user_id" class="custom-select w-full custom-select-lg" v-model="form.user_id">
                          <option value="" disabled selected>Nurse</option>
                ...
                <div class="vx-col w-full mt-4"  style="text-align:center;">
                    <vs-button v-show="!editMode" ref="loadableButton" id="button-with-loading" class="vs-con-loading__container" @click="createNurse();" type="relief">Create</vs-button>
                    <vs-button v-show="editMode"  ref="loadableButton" id="button-with-loadings" class="vs-con-loading__container" @click="updateNurse();" type="relief">Update</vs-button>
                </div>
            </div>
            </form>
 </div>
...
export default {
  ...
  methods: {
    ...
    showCreateSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Nurse has Created',
        text: 'Nurse has created successfully!'
      })
    },
    showUpdatedSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Nurse has Updated',
        text: 'Nurse has Updated successfully!',
        globalPosition: 'top right',
      })
    },
    createNurse () {
      this.$store.dispatch('addNurse', Object.assign({}, this.form))
      this.clearFields()
      this.showCreateSuccess()
    },
    updateNurse(){
         this.$store.dispatch('updateNurse', Object.assign({}, this.form))
