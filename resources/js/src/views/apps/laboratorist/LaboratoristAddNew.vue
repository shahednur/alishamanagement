<!-- =========================================================================================
  File Name: LaboratoristAddNew.vue
  Description: Laboratorist module add new
  ----------------------------------------------------------------------------------------
...
<template>
 <div class="vx-col md:w-1/1 w-full mb-base">
  <form  @submit.prevent="editModal ? updateLaboratorist() : createLaboratorist()">
    <div class="vx-row">
        ...
        <div class="vx-row">
          <div class="vx-col md:w-1/2 w-full">
            ...
            <select id="nurses" name="user_id" class="custom-select w-full custom-select-lg" v-model="form.user_id">
                <option value="" disabled selected>Laboratorist</option>
      ...
      <div class="vx-col w-full mt-4"  style="text-align:center;">
          <vs-button v-show="!editMode" ref="loadableButton" id="button-with-loading" class="vs-con-loading__container" @click="createLaboratorist();" type="relief">Create</vs-button>
          <vs-button v-show="editMode"  ref="loadableButton" id="button-with-loadings" class="vs-con-loading__container" @click="updateLaboratorist();" type="relief">Update</vs-button>
      </div>
  </div>
  </form>
 </div>
...
export default {
  ...
  methods: {
    ...
    showCreateSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Laboratorist has Created',
        text: 'Laboratorist has createdsuccessfully!'
      })
    },
    showUpdatedSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Laboratorist has Updated',
        text: 'Laboratorist has Updated successfully!',
        globalPosition: 'top right',
      })
    },
    createLaboratorist() {
      this.$store.dispatch('addLaboratorist', Object.assign({}, this.form))
      this.clearFields()
      this.showCreateSuccess()
    },
    updateLaboratorist(){
         this.$store.dispatch('updateLaboratorist', Object.assign({}, this.form))
