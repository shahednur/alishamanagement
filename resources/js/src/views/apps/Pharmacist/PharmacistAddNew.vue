...
<template>
 <div class="vx-col md:w-1/1 w-full mb-base">
            <form  @submit.prevent="editModal ? updatePharmacist() : createPharmacist()">
              <div class="vx-row">
                  ...
                  <div class="vx-row">
                    <div class="vx-col md:w-1/2 w-full">
                      ...
                      <select id="nurses" name="user_id" class="custom-select w-full custom-select-lg" v-model="form.user_id">
                          <option value="" disabled selected>Pharmacist</option>
                ...
                <div class="vx-col w-full mt-4"  style="text-align:center;">
                    <vs-button v-show="!editMode" ref="loadableButton" id="button-with-loading" class="vs-con-loading__container" @click="createPharmacist();" type="relief">Create</vs-button>
                    <vs-button v-show="editMode"  ref="loadableButton" id="button-with-loadings" class="vs-con-loading__container" @click="updatePharmacist();" type="relief">Update</vs-button>
                </div>
            </div>
            </form>
 </div>
...
export default {
  ...
  methods: {
    ...
    showCreateSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Pharmacist has Created',
        text: 'Pharmacist has createdsuccessfully!'
      })
    },
    showUpdatedSuccess () {
      this.$vs.notify({
        color: 'success',
        title: 'Pharmacist has Updated',
        text: 'Pharmacist has Updated successfully!',
        globalPosition: 'top right',
      })
    },
    createPharmacist() {
      this.$store.dispatch('addPharmacist', Object.assign({}, this.form))
      this.clearFields()
      this.showCreateSuccess()
    },
    updatePharmacist(){
         this.$store.dispatch('updatePharmacist', Object.assign({}, this.form))
